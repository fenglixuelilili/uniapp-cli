<template>
  <view class="container">
    <view class="page-body uni-content-info">
      ...
      <view class="cropper-config" v-if="false">
        <button
        ...
        <button type="warn" @click="getImageInfo" style="margin-top: 30rpx;">
          点击生成图片
        </button>
      </view>
      <canvas
    ...
    <button type="warn" @click="getImageInfo" class="sure" size="mini">
      确定
    </button>
  </view>
</template>
...
export default {
  ...
  methods: {
    init(options) {
      const { src, width, height, routename } = options
      this.imageSrc = src
      this.routename = routename
      setTimeout(() => {
        IMG_RATIO1 = width / height
        this.loadImage()
      }, 0)
    },
    ...
    loadImage: function() {
      ...
      uni.getImageInfo({
        ...
        success: function success(res) {
          ...
          // 根据图片比例和裁剪图片比例 决定    按照图片的高度为裁剪高度    或  按照图片的宽度为裁剪高度
      ...
      ctx.draw(true, () => {
        ...
        uni.canvasToTempFilePath({
          ...
          success: function(res) {
            ...
            // 路由
            setTimeout(() => {
              upLoad('image', tempFilePath).then(data => {
                // console.log(data)
                let params = ''
                for (let key in _this.opetions_) {
                  if (key !== 'src' && key !== 'srcpath') {
                    params += `&${key}=${_this.opetions_[key]}`
                  }
                }
                uni.redirectTo({
                  url:
            ...
            // console.log(_this.routename + '?src=' + tempFilePath, '路由')
    ...
    dragMove(e) {
      ...
      switch (dragType) {
        case 'rightBottom':
          ...
          let cutB =
            IMG_REAL_H -
            (IMG_REAL_W - this.cutL - cutR) / IMG_RATIO1 -
            this.cutT
          if (cutR <= 0 || cutB <= 0) {
            return
          }
